...
→ Select all of its ready datasets
→ Create an analysis run linked to those datasets (dashboard_generating)
...
- The dashboard service selects all ready workspace datasets, creates an analysis run linked to them, invokes the graph, and maps graph state to the public response schema.
...
- The configured Ollama client gives the supervisor, KPI, anomaly, forecast, insight, layout, and chat agents separate model settings in `agent_models.toml`. Deterministic code detects anomalies; the anomaly agent only explains the resulting evidence.
...
1. Select every workspace dataset with `status = ready`.
2. Insert an `analysis_runs` record and an `analysis_run_datasets` link for each selected dataset, with status `dashboard_generating`.
3. Invoke the compiled dashboard graph with `analysis_id` and `dataset_ids`.
...
Each analysis run is linked to every ready dataset in its workspace through `analysis_run_datasets`. `load_dataset_context` verifies those links and workspace ownership, then returns each dataset's metadata and ordered field definitions. The dashboard agents use `dataset.field` references for selected fields so identically named fields remain unambiguous.
...
|---|---|
| `load_dataset_context` | Verifies every analysis-to-dataset link, then loads the linked dataset metadata and ordered field definitions from Supabase. |
...
| `detect_anomalies` | Loads each authorised dataset Parquet file, applies Tukey's 1.5 IQR fences to every numeric column, then asks the anomaly LLM to explain the fixed evidence. |
...
The KPI/trend and forecast nodes are currently LLM-backed. They do not execute DuckDB queries or invoke deterministic analytical or forecasting models, so their numeric values and SQL strings are not independently verified by the backend. The anomaly node reads the stored Parquet file and calculates IQR fences deterministically.
...
- `analysis_runs`: the workspace, status, creation time, and chart layout.
- `analysis_run_datasets`: the immutable dataset links selected for an analysis run.
...
| `POST /dashboard` | Implemented | Generates a dashboard synchronously from every ready dataset in the authenticated user's workspace. |
...
1. Deterministic KPI, trend, and forecast calculations over the processed Parquet data.
